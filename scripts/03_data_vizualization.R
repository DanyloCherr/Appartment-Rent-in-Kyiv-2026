...
temp_all_tables <- all_tables
for(name in names(temp_all_tables)){
  curr_table <- temp_all_tables[[name]]
  temp_all_tables[[name]] <- curr_table[curr_table$Дата >= "2015-11-01", ]
}

combined_plots(temp_all_tables[c("GPR_INDEX", "FS_INDEX")])

temp_all_tables[names(temp_all_tables)]
names(temp_all_tables)
